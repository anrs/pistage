...
type Store interface {
	...
	NewMutex(key string) (sync.Locker, error)
}
...
func New(metatype string) (Store, error) {
	...
	case "etcd":
		return etcd.New()
	default:
		return nil, errors.Annotatef(errors.ErrInvalidValue, "unsupported meta: %s", metatype)
	}
}
...
// Lock .
func Lock(ctx context.Context, key string) (sync.Unlocker, error) {
